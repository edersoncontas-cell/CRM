 ...
 *
 * Sem body: percorre TODAS as conversas sem nome e tenta corrigir via CRM + Z-API (LID e phone).
 * Com body { phone, nome }: corrige pontualmente UMA conversa pelo telefone.
 */
export async function POST(req: Request) {
  try {
    const body = await req.json().catch(() => null) as { phone?: string; nome?: string } | null;

    // ── Correção pontual por telefone ──
    if (body?.phone && body?.nome) {
      const nome = body.nome.trim();
      const variants = phoneLookupVariants(body.phone);
      // Busca também pelo próprio valor (pode ser LID)
      const phoneDigits = body.phone.replace(/\D/g, "");
      const updated = await db.whatsAppConversation.updateMany({
        where: {
          OR: [
            { externalPhone: { in: [...variants, body.phone] } },
            { lid: `${phoneDigits}@lid` },
            { lid: body.phone },
          ],
        },
      ...
      return NextResponse.json({
        ...
        mensagem: `Conversa de ${body.phone} atualizada para "${nome}" (${updated.count} registros).`,
      });
    }
    ...
    for (const conv of precisaCorrigir) {
      ...
      // 4. Z-API por LID (números 14-15 dígitos são LIDs do WhatsApp)
      if (!nomeNovo) {
