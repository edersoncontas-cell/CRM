  ...
  try {
    ...
    if (body?.phone && body?.nome) {
      ...
      return NextResponse.json({
        ...
        mensagem: `Conversa de ${body.phone} → "${nome}" (${updated.count} registros).`,
      });
    }
    ...
    return NextResponse.json({
      ...
      mensagem: `${precisaCorrigir.length} sem nome → ${precisaCorrigir.length - semNome} corrigidos. ${semNome} sem match.`,
    });
  } catch (e) {
