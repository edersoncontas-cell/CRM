...
import { coresSerie, type TemaDash } from "@/lib/dash-tema";
import { useTemaDash } from "@/components/TemaDashProvider";

// Eram constantes de módulo: congelavam as cores do tema escuro e os eixos
// saíam cinza-claro dentro do tema claro. Agora derivam do tema recebido.
const eixo = (T: TemaDash) => ({ fill: T.texto2, fontSize: 11 });
const tooltipStyle = (T: TemaDash) => ({ background: T.card2, border: `1px solid ${T.borda}`, borderRadius: 12, color: T.texto, fontSize: 12 });
  ...
  return (
    <ResponsiveContainer width="100%" height={240}>
      <ComposedChart data={comMeta} margin={{ top: 10, right: 6, left: -12, bottom: 0 }}>
        ...
        <XAxis dataKey="mes" tick={eixo(T)} axisLine={false} tickLine={false} />
        <YAxis yAxisId="valor" tick={eixo(T)} axisLine={false} tickLine={false} tickFormatter={compacto} width={60} />
        <YAxis yAxisId="qtd" orientation="right" tick={eixo(T)} axisLine={false} tickLine={false} allowDecimals={false} width={28} />
        <Tooltip
  ...
  return (
    <ResponsiveContainer width="100%" height={200}>
      <BarChart data={dados} margin={{ top: 22, right: 8, left: -12, bottom: 0 }}>
        <CartesianGrid stroke={T.borda} vertical={false} />
        <XAxis dataKey="ano" tick={eixo(T)} axisLine={false} tickLine={false} />
        <YAxis tick={eixo(T)} axisLine={false} tickLine={false} tickFormatter={compacto} width={60} />
        <Tooltip contentStyle={tooltipStyle(T)} formatter={(v: number) => [formatCurrency(v), "Ticket médio"]} cursor={{ fill: T.sobre }} />
    ...
    <div className="flex items-center gap-3">
      <ResponsiveContainer width={130} height={130}>
        <PieChart>
          ...
          <Tooltip contentStyle={tooltipStyle(T)} formatter={(v: number, n: string) => [`${v} (${Math.round((v / total) * 100)}%)`, n]} />
        </PieChart>
      </ResponsiveContainer>
      <ul className="min-w-0 flex-1 space-y-1.5 text-xs">
        {dados.slice(0, 6).map((d, i) => (
          <li key={d.nome} className="flex items-center gap-2" style={{ color: T.texto2 }}>
            <span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: CORES[i % CORES.length], boxShadow: `0 0 8px ${CORES[i % CORES.length]}` }} />
  ...
  return (
    <ResponsiveContainer width="100%" height={altura}>
      <BarChart data={dados} layout="vertical" margin={{ top: 0, right: 30, left: 0, bottom: 0 }} barCategoryGap={7}>
        ...
        <YAxis type="category" dataKey="nome" width={112} tick={{ ...eixo(T), fill: T.texto2 }} axisLine={false} tickLine={false} />
        <Tooltip contentStyle={tooltipStyle(T)} formatter={(v: number) => [v, rotulo]} cursor={{ fill: T.sobre }} />
