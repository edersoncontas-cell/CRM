      ...
      {itens.length === 0 ? (
        ...
        <ul className="grid grid-cols-1 gap-1.5 md:grid-cols-2">
          {itens.slice(0, 10).map((n, i) => (
            <li key={i}>
              <a href={n.link} target="_blank" rel="noreferrer" className="flex items-start gap-2 rounded-xl px-3 py-2 text-sm transition hover:brightness-125" style={{ background: "rgba(255,255,255,0.04)" }}>
