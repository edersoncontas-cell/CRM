...
function Coluna({ id, titulo, cards }: { id: string; titulo: string; cards: CardData[] }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "flex w-72 shrink-0 flex-col rounded-xl bg-slate-100 p-3",
    ...
    >
      ...
      <div className="mb-2 text-xs text-slate-400">{formatCurrency(total)}</div>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        (isDragging || arrastando) && "opacity-80 shadow-lg"
      )}
    >
      <div {...listeners} {...attributes} className="cursor-grab active:cursor-grabbing">
        <div className="flex items-center justify-between">
          <span className="font-medium text-slate-800">{card.cliente}</span>
          {card.maquina && (
        ...
        <div className="mt-1 text-xs text-slate-500">{card.municipio ?? "Sem município"}</div>
        <div className="mt-1 text-sm font-semibold text-green-600">{formatCurrency(card.valor)}</div>
        {card.concorrente && (
          <div className="mt-1 text-xs text-amber-600">⚔ {card.concorrente}</div>
        )}
      ...
      {!arrastando && (
        <div className="mt-2 flex gap-2 border-t border-slate-100 pt-2">
          <form action={marcarGanha.bind(null, card.id)}>
            <button className="text-xs font-medium text-green-600 hover:underline">Ganhou</button>
          </form>
