...
export function AssistenteIA() {
  ...
  return (
    <>
      ...
      >
        <span className="absolute -right-0.5 -top-0.5 flex h-5 w-5 items-center justify-center rounded-full bg-black text-[9px] font-black text-agro-400">IA</span>
        <ExcavatorIcon size={26} className="text-black" />
        {!aberto && !pos && <span className="absolute inset-0 animate-ping rounded-full" style={{ background: "rgba(250,204,21,0.35)" }} />}
      </button>
      ...
      {aberto && (
        ...
        >
          {/* Header */}
          <div className="flex items-center gap-2 px-4 py-3" style={{ background: "#eab308" }}>
            <ExcavatorIcon size={20} className="text-black" />
            <div className="flex-1">
              <div className="text-sm font-bold text-black">Assistente IA 🚜</div>
              <div className="text-[10px] text-black/70">Fale ou escreva — crio, edito, analiso, sem limites</div>
            </div>
            <button onClick={() => setAberto(false)} className="rounded-lg p-1 text-black/70 hover:bg-black/10 hover:text-black">
              <X size={18} />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {fase === "idle" && !feedback && (
              <p className="text-xs text-slate-500 leading-relaxed">
                Pode pedir qualquer coisa, sem limites: cadastrar clientes, criar ou mover negociações, marcar venda ganha/perdida, agendar visitas, atualizar dados, criar tarefas... só falar ou ditar.
              </p>
            )}
            ...
            {fase === "feito" && (
              <div className="space-y-3">
                ...
                <button onClick={reset} className="text-xs font-semibold text-yellow-400 hover:underline">Nova mensagem</button>
              </div>
            )}
          </div>
