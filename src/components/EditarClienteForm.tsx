                  ...
                  <div
                            ...
                            <form
                                        ...
                                        <div className="mb-4 flex items-center justify-between">
                                                      <h2 className="text-lg font-bold text-slate-800">Editar cliente</h2>h2>
                                                      <button type="button" onClick={fechar}><X className="text-slate-400" /></button>button>
                                        </div>div>
                                        ...
                                        <div className="space-y-3">
                                                      ...
                                                      <Campo label="Cidade / região">
                                                                ...
                                                                <option key={m.id} value={m.id}>{m.nome}</option>option>
                                                              ))}
                                                                                          </optgroup>optgroup>
                                          ...
                                          {status === "cliente" && (
                                                          <div className="rounded-xl border border-blue-200 bg-blue-50 p-3 space-y-2" style={{ overflow: "hidden" }}>
                                                                            <div className="text-sm font-semibold text-blue-800">🚜 Frota do cliente</div>div>
                                                            {frota.length > 0 && (
                                                                                <ul className="space-y-1">
                                                                                  {frota.map((f) => (
                                                                                                          <li key={f.id} className="flex items-center justify-between rounded-lg bg-white px-3 py-1.5 text-sm border border-blue-100">
                                                                                                                                    <span className="font-medium text-slate-700">{f.marca} — {f.modelo}</span>span>
                                                                            ...
                                                                            <div className="flex gap-2">
                                                                                                <select
                                                                                                                      ...
                                                                                                                      >
                                                                                                  {marcas.map((m) => <option key={m} value={m}>{m}</option>option>)}
                                                                                                  </select>select>
                                                                              {modelosPorMarca.length > 0 ? (
                                                                                  <select
                                                                                    ...
                                                                                    {modelosPorMarca.map((m) => <option key={m} value={m}>{m}</option>option>)}
                                                                                                          <option value="__outro__">Outro (digitar)</option>option>
                                                                                    </select>select>
                                                                                ) : (
                                                                                  <input
                                                                        ...
                                                                        font-size: .875rem;
                                                                                  outline: none;
                                                                                            box-sizing: border-box;
                                                                                                      max-width: 100%;
                                                                                                                /* Remove resize handle que aparece como pontinhos no iOS */
                                                                                                                          resize: none;
...
function Campo({ label, children }: { label: string; children: React.ReactNode }) {
    return (
          <div style={{ width: "100%", overflow: "hidden" }}>
                <label className="mb-1 block text-sm font-medium text-slate-700">{label}</label>label>
            {children}
          </div>div>
        );
}</>
